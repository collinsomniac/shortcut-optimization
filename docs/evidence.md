...
Each substantive capability claim needs a scope, evidence level, and source or reproduction record.
...
| unknown | Insufficient evidence, including a search that found no answer |

Use labels per claim, not per app. One successful run does not establish general support. A failed search does not establish impossibility.

Record date, hardware, exact iOS build, app versions, language/region, AI availability/model choice, source app, permissions, foreground/locked state, network and sync conditions, shortcut version/hash, input, expected/actual output, attempts, and evidence attachments. Redact private material.
