...
export class AtlasView{
 constructor(host,atlas,onSelect){
  ...
  this.canvas=document.createElement('canvas');host.append(this.canvas);this.scene=new THREE.Scene();this.camera=new THREE.PerspectiveCamera(42,1,.1,100);this.camera.position.set(.9,.625,1.8);
  ...
  this.leaders=document.createElementNS('http://www.w3.org/2000/svg','svg');this.leaders.setAttribute('class','anchor-leaders');host.append(this.leaders);
 ...
 reset(){this.camera.position.set(.9,.625,1.8);this.controls.target.set(0,0,0);this.controls.update();this.invalidate();}
 ...
 render(){if(!this.width||document.hidden)return;this.camera.updateMatrixWorld();const boxes=[];this.leaders.replaceChildren();this.leaders.setAttribute('viewBox',`0 0 ${this.width} ${this.height}`);
  this.labels.forEach((el,i)=>{const p=this.screen(this.atlas.anchors[i].position);el.style.display=p.visible?'block':'none';if(!p.visible)return;const w=el.offsetWidth,h=el.offsetHeight;let box;
   for(let attempt=0;attempt<48;attempt++){const angle=attempt*2.4,r=12+Math.floor(attempt/6)*18,x=Math.max(3,Math.min(this.width-w-3,p.x+Math.cos(angle)*r-w/2)),y=Math.max(3,Math.min(this.height-h-32,p.y+Math.sin(angle)*r-h-5));box={x,y,w,h};if(!boxes.some(b=>x<b.x+b.w+5&&x+w+5>b.x&&y<b.y+b.h+4&&y+h+4>b.y))break;}
   boxes.push(box);el.style.transform=`translate(${box.x}px,${box.y}px)`;const line=document.createElementNS('http://www.w3.org/2000/svg','line');for(const [k,v] of Object.entries({x1:p.x,y1:p.y,x2:box.x+box.w/2,y2:box.y+box.h/2}))line.setAttribute(k,v);this.leaders.append(line);
