...
export function validateReplay(d,atlas){
 if(d.atlasVersion!==atlas.version||d.revision!==atlas.revision||d.hook!==atlas.hook)throw Error('Trace uses a different coordinate space');
 if(!Array.isArray(d.events)||d.events.length>512)throw Error('Trace must contain at most 512 events');
 for(const e of d.events)if(e.position?.length!==3||e.similarities?.length!==atlas.anchors.length||!e.position.every(Number.isFinite)||!e.similarities.every(Number.isFinite)||typeof e.text!=='string')throw Error('Invalid trace measurements');
 return d;
}
