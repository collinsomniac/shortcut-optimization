...
function render(){
 ...
 $('interpretation').textContent=e&&e.projectionRetained<.1?'Most activation variation is outside this 3D view. Prompt-defined landmarks may poorly describe decoding states.':'Landmarks are reference prompts. Lines show token order, not causal influence.';
...
$('load').onclick=()=>{status('Preparing WebGPU model…');worker=new Worker(new URL('./atlas-worker.js?v='+__ATLAS_BUILD__,import.meta.url),{type:'module'});controls();worker.onerror=e=>{status(`Worker failed: ${e.message}`);release();};worker.onmessage=({data:m})=>{if(m.type==='status')status(m.text);if(m.type==='ready'){ready=true;controls();status('Ready. Each generated token captures one middle-layer residual state.');}if(m.type==='error'){status(m.message);release();}if(m.type==='token'){const r=runs.find(r=>r.id===liveId);if(!r)return;r.events.push(m.event);if(follow)selected={runId:r.id,step:r.events.length-1};render();}if(m.type==='done'){busy=false;controls();if(!m.failed)status('Capture complete. Scrub, compare, or save this trace.');}};worker.postMessage({type:'load'});};
